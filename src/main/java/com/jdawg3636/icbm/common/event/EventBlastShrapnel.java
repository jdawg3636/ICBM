...
public class EventBlastShrapnel extends AbstractBlastEvent {
    ...
    public EventBlastShrapnel(BlockPos blastPosition, ServerWorld blastWorld, AbstractBlastEvent.Type blastType) {
    ...
    public boolean executeBlast() {
        ...
        for(double i = -0.5; i <= 0.5; i += 0.0625) {
            for(double j = -0.5; j <= 0.5; j += 0.0625) {
                ...
                EntityShrapnel shrapnelEntity = new EntityShrapnel(getBlastWorld(), getBlastPosition().getX(), getBlastPosition().getY(), getBlastPosition().getZ(), false, getBlastType());
