...
public class BlockEditHandler {
    ...
    public static final HashMap<RegistryKey<World>, Queue<EditQueue>> worldToRemoveQueue = new HashMap<>();
    ...
    public static void onWorldUnload(WorldEvent.Unload event) {
        final RegistryKey<World> dim = ((World)event.getWorld()).func_234923_W_();
    ...
    public static void onWorldTick(TickEvent.WorldTickEvent event) {
        final RegistryKey<World> dim = event.world.func_234923_W_();
        if (worldToRemoveQueue.containsKey(dim)) {
    ...
    public static void queue(World world, Collection<BlockPos> edits, Consumer<BlockPos> onEditBlock, Runnable onCompleteCallback) {
        final RegistryKey<World> dim = world.func_234923_W_();
        if (!worldToRemoveQueue.containsKey(dim)) {
