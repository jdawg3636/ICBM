...
public class PacketManager {
    ...
    protected EnumMap<Dist, FMLEmbeddedChannel> channelEnumMap;
    ...
    public void sendToPlayer(IPacket packet, ServerPlayerEntity player) {
        ...
        if (channelEnumMap != null) {
            this.channelEnumMap.get(Dist.DEDICATED_SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGET).set(FMLOutboundHandler.OutboundTarget.PLAYER);
            this.channelEnumMap.get(Dist.DEDICATED_SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGETARGS).set(player);
            this.channelEnumMap.get(Dist.DEDICATED_SERVER).writeAndFlush(packet);
        }
        else {
    ...
    public void sendToAllInDimension(IPacket packet, int dimId) {
        ...
        //Null check is for JUnit
        if (channelEnumMap != null) {
            this.channelEnumMap.get(Dist.DEDICATED_SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGET).set(FMLOutboundHandler.OutboundTarget.DIMENSION);
            this.channelEnumMap.get(Dist.DEDICATED_SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGETARGS).set(dimId);
            this.channelEnumMap.get(Dist.DEDICATED_SERVER).writeAndFlush(packet);
        }
        else {
    ...
    public void sendToAll(IPacket packet) {
        ...
        //Null check is for JUnit
        if (channelEnumMap != null) {
            this.channelEnumMap.get(Dist.DEDICATED_SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGET).set(FMLOutboundHandler.OutboundTarget.ALL);
            this.channelEnumMap.get(Dist.DEDICATED_SERVER).writeAndFlush(packet);
        }
        else {
    ...
    public void sendToAllAround(IPacket message, NetworkRegistry.TargetPoint point) {
        ...
        //Null check is for JUnit
        if (channelEnumMap != null) {
            this.channelEnumMap.get(Dist.DEDICATED_SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGET).set(FMLOutboundHandler.OutboundTarget.ALLAROUNDPOINT);
            this.channelEnumMap.get(Dist.DEDICATED_SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGETARGS).set(point);
            this.channelEnumMap.get(Dist.DEDICATED_SERVER).writeAndFlush(message);
        }
        else {
    ...
    public void sendToServer(IPacket packet) {
        ...
        //Null check is for JUnit
        if (channelEnumMap != null) {
            this.channelEnumMap.get(Dist.CLIENT).attr(FMLOutboundHandler.FML_MESSAGETARGET).set(FMLOutboundHandler.OutboundTarget.TOSERVER);
            this.channelEnumMap.get(Dist.CLIENT).writeAndFlush(packet);
        }
        else {
