    ...
    public boolean doExplode(int callCount) /* TODO Rewrite this entire method */ {
                        ...
                        if (distance <= radius) {
                            ...
                            if (!block.isAir(blockState, world, targetPosition) && blockState.getBlockHardness(world, targetPosition) >= 0) {
                                ...
                                MinecraftForge.EVENT_BUS.post(new BlastBlockModifyEvent(world, targetPosition, () -> {
                                        ...
                                        //Destroy block
                                        this.world().setBlockState(targetPosition, Blocks.AIR.getDefaultState());

                                        //Create floating block
                                        if (
                                            !(block instanceof FlowingFluidBlock || block instanceof IFluidBlock)
        ...
        for (Entity entity : allEntities) {
            ...
            if (entity instanceof IMissile)
                ...
                //Get difference
                double xDelta = entity.getPosX() - location.x();
                double yDelta = entity.getPosY() - location.y();
                double zDelta = entity.getPosZ() - location.z();
