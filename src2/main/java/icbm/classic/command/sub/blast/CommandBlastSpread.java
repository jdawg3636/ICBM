...
public class CommandBlastSpread extends SubCommand {
    ...
    public int handleCommand(CommandContext<CommandSource> context) throws CommandException {
        if (args.length == 8)
            ...
            throw new (ICBMCommands.TRANSLATION_UNKNOWN_COMMAND, getUsage(sender));
    }
    ...
    private void doCommand(@Nonnull CommandSource sender, @Nonnull String[] args) throws CommandException {
        ...
        //Get world position
        final World world = CommandUtils.getWorld(sender, args[3], sender.getEntity().getEntityWorld());
        final double xInput = CommandUtils.getNumber(sender, args[4], sender.getPos().getX() + 0.5);
        final double yInput = CommandUtils.getNumber(sender, args[5], sender.getPos().getY() + 0.5);
        final double zInput = CommandUtils.getNumber(sender, args[6], sender.getPos().getZ() + 0.5);
        ...
        for (int xi = -count; xi <= count; xi++) {
            ...
            for (int zi = -count; zi <= count; zi++) {
                ...
                if(result != BlastState.TRIGGERED && result != BlastState.THREADING) {
                    ...
                    sender.sendFeedback(
                            new TranslationTextComponent(
                                    CommandBlastTrigger.getTranslationKey(result),
                                    explosiveData.getRegistryName(), scale,
                                    world.provider.getDimension(), world.getWorldType().getName(),
                                    x, yInput, z
                            ),
