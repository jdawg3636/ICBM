...
public class BlastAntiGravitational extends BlastThreaded implements IBlastTickable {
    ...
    protected static EntityFlyingBlock sendBlockFlying(World world, BlockPos targetPosition, BlockState blockState) {
        ...
        entity.setMotion(entity.getMotion().add(0, Math.max(0.15 * world.rand.nextFloat(), 0.1), 0));
        entity.noClip = true;
    ...
    public boolean doExplode(int callCount) /* TODO rewrite entire method */ {
                    ...
                    if (this.thread.isComplete) {
                        ...
                        for (BlockPos targetPosition : results) {
                            ...
                            if (!blockState.getBlock().isAir(blockState, world, targetPosition) //don't pick up air
                                && !blockState.getMaterial().isReplaceable() //don't pick up irreplaceable blocks like fire, grass, or snow (this does not include crops)
                                && !(blockState.getBlock() instanceof IFluidBlock) && !(blockState.getBlock() instanceof FlowingFluidBlock)
                                ) /* don't pick up liquids */ {
        ...
        for (Entity entity : allEntities) {
            if (!(entity instanceof EntityFlyingBlock) && entity.getPosY() < 100 + location.y()) {
                if (entity.getMotion().getY() < 0.4) {
                    entity.setMotion(entity.getMotion().add(0, 0.15, 0));
                }
            }
        }
