...
public class BlastRedmatter extends Blast implements IBlastTickable, IBlastMovable {
    ...
    private boolean shouldHandleEntity(Entity entity) {
        ...
        if (entity instanceof PlayerEntity && (((PlayerEntity) entity).isCreative() || ((PlayerEntity) entity).abilities.disableDamage))
            return false;
    ...
    protected void handleEntities(Entity entity) /* TODO why is radius and doExplosion not used */ {
        ...
        final double xDifference = entity.getPosX() - location.xi() + 0.5; //TODO why 0.5? blast might actually have decimal position
        final double yDifference = entity.getPosY() - location.yi() + 0.5;
        final double zDifference = entity.getPosZ() - location.zi() + 0.5;
